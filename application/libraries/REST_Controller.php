...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    {
        $status = $this->db->insert(config_item('rest_logs_table'), array(
         ...
         // $status = $this->rest->db->query('insert into '.config_item('rest_logs_table').
                    // '(uri,method,params,api_key,ip_address,time,authorized)'.
                    // 'values ('.
                    // $this->uri->uri_string().
        ...
        $rs = $this->db->get(config_item('rest_logs_table'));
        $id = -1;
        if ($rs && $rs->num_rows()) {
            $id = $rs->first_row()->ID;
        }
